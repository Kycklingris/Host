class_name V1_Lobby
extends Node
var _internal: _V1_LobbyInternal

enum State { NOT_INITIALIZED, FAILED, LOBBY_CREATED, LOBBY_SDP_SET }

signal lobby_created
signal player_joined(player: V1_Player)

var id: String:
	get:
		if self._internal != null and self._internal.id != null:
...
@export var game: String:
	get:
		if self._internal != null and self._internal.game != null:
			return String(self._internal.game)
		#printerr("Tried to access lobby variable \"game\" before initialization")
		return game
	set(value):
		if self._internal == null or self._internal.game == null:
			game = value


@export var min_players: int:
	get:
		if self._internal != null and self._internal.min_players != null:
...
func with_data(p_game: String = "", p_min_players: int = 0, p_max_players: int = 0, allow_audience: bool = false):
	self.game = p_game
...
func _ready():
	self._internal = _V1_LobbyInternal.new(game, min_players, max_players, false)
...
class _V1_LobbyInternal:
	...
	var placeholder_players: Array[V1_Player];
	...
	func _delete():
		...
		for player in self.players:
			player._internal.channel._send_magic(V1_Player._V1_PlayerInternal._Channel._InternalMagicByte.DISCONNECT);
	...
	func _new_lobby():
		...
		for i in self.max_players:
			var new_player = V1_Player.new(self.id, self.id, turn_password, i);
		...
		self._set_state(V1_Lobby.State.LOBBY_SDP_SET);
		...
		self._set_state(V1_Lobby.State.LOBBY_CREATED);
	...
	func _player_connected(player: V1_Player):
