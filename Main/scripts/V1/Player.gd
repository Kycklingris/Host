class_name _PlayerInternalV1
extends Node

signal _connected(player: V1.Player);
signal _sdp_complete();

var outer = V1.Player;

var current_state: V1.Player.State = V1.Player.State.CREATED;
...
func _init(p_lobby_id: String, p_turn_username: String, p_turn_password: String, p_index: int, p_outer: V1.Player):
	...
	self.root_element = V1.Element.new("div");
...
func _initialize():
	self.current_state = V1.Player.State.CREATED;
	...
	self.current_state = V1.Player.State.GATHERING_SDP;
	...
	self.current_state = V1.Player.State.POLLING;
	self._poll();
	return;

func _poll():
	if (self.current_state != V1.Player.State.POLLING):
		return;
...
func _set_data(data: Dictionary):
	...
	self.current_state = V1.Player.State.WAITING_FOR_CONNECTION;
	return;
...
func _process(_delta):
	...
	if (self.current_state == V1.Player.State.GATHERING_SDP and gathering_state == WebRTCPeerConnection.GATHERING_STATE_COMPLETE):
		self.current_state = V1.Player.State.SET_SDP;
		self._sdp_complete.emit();
	
	if (self.current_state == V1.Player.State.WAITING_FOR_CONNECTION and connection_state == WebRTCPeerConnection.STATE_CONNECTED):
		self.current_state = V1.Player.State.CONNECTED;
		self._on_connected();
		
	if (self.current_state == V1.Player.State.CONNECTED and connection_state == WebRTCPeerConnection.STATE_CLOSED):
		self.current_state = V1.Player.State.FAILED;
		print("DISCONNECTED!");
		_initialize();
